...
help:
	...
	@echo "    make install-dev   install Python tools used by this Makefile from requirements-dev.txt"
...
.PHONY: install-dev
install-dev:
	@echo "The following installation assumes an active conda environment with Python 3.6 on an Ubuntu system."
	@echo
	# System dependency of plotly-orca when used with tox
	...
	pip install pandas pillow pytest pytest-cov pytest-timeout tox
	# Documentation generation
	# - NLTK for example with linguistic graph
	# - sphinx<2 to prevent a translation error of numpy style docstrings into HTML
	# - sphinx_rtd_theme==0.4.3 to ensure manually modified theme.js fits to rest
	# - tornado<6 to prevent a websocket error with Jupyter notebooks
	pip install nltk "sphinx<2" "sphinx_rtd_theme==0.4.3" "tornado<6" jupyter nbsphinx urlchecker
	# Upload to PyPI
	pip install twine
...
clean:
    # Directories
	-@rm -rf docs/build
	-@rm -rf tests/output* tests/htmlcov tests/.coverage*
